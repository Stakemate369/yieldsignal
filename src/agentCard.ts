...
const AGENT_CARD = {
  ...
  description:
    "Real-time, risk-weighted yield signals for autonomous agents: ETH liquid staking APY across Lido, Rocket Pool, Coinbase Wrapped Staked ETH, Frax Ether and Binance Staked ETH on Ethereum mainnet, plus USDC/WETH lending APY across Aave, Compound, Morpho, Moonwell, Euler and Fluid on Base — sold per-call via x402, REST + MCP, no API key. Seven products: the raw signal; a buyer-side MOVE/HOLD decision with expected net gain and break-even in days; a durability report splitting each APY into base vs incentive and reporting the post-incentive floor; an exit-capacity report giving per-protocol utilization and withdrawable liquidity from the protocol's own books; a rate-sensitivity report giving each market's distance to the kink where borrow rates explode, read from the protocol's own interest rate curve; and a shared-exposure report that takes declared positions and shows how much capital sits behind the same collateral, oracle or curator across venues; and a leadership-persistence report — the one no competing feed can produce — measuring, from this service's own hourly on-chain attestations, how long its calls actually held: median lead duration, survival at 6/24/72h, the round-trip share of leader rotation, and the measured finding that the size of a lead does NOT predict its durability. Every response signed (EIP-712 typed data) by the payment-receiving address; periodic on-chain attestations (EAS, Base mainnet) provide a public, permanent track record independent of this server's uptime, per-asset verified accuracy is free at /accuracy.json, and the aggregate leadership-durability history — including the measured finding that a wider yield lead is NOT more durable — is free and reproducible at /persistence.json.",
  services: [
