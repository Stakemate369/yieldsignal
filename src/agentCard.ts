...
const AGENT_CARD = {
  ...
  description:
    "Real-time, risk-weighted yield signals for autonomous agents: ETH liquid staking APY across Lido, Rocket Pool, Coinbase Wrapped Staked ETH, Frax Ether and Binance Staked ETH on Ethereum mainnet, plus USDC/WETH lending APY across Aave, Compound, Morpho, Moonwell, Euler and Fluid on Base — sold per-call via x402, REST + MCP, no API key. Four products: the raw signal; a buyer-side MOVE/HOLD decision with expected net gain and break-even in days; a durability report splitting each APY into base vs incentive and reporting the post-incentive floor; and an exit-capacity report giving per-protocol utilization and withdrawable liquidity from the protocol's own books. Every response signed (EIP-712 typed data) by the payment-receiving address; periodic on-chain attestations (EAS, Base mainnet) provide a public, permanent track record independent of this server's uptime, and per-asset verified accuracy is free at /accuracy.json.",
  services: [
